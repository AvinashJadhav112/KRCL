...
export class AlertsDevices extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState({ loading: true }, async () => {
      ...
      const AlertsResult = await axios.all(this.state.filteredIds.map((id) => axios.get(`http://192.168.0.194:5005/api/1.0/alert/alertCount/${id}?start=${this.state.currentDate}T00:00:00Z&end=${this.state.currentDate}T23:59:59Z`)));
    ...
    return (
      ...
      >
        ...
        <div className="row">
          ...
          {this.state.AlertFiltered.length === 0
            ? (
