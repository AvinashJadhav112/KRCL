      ...
      >
        ...
        <div className="row">
          ...
          {this.state.sensorReadingTwo.map((di) => {
            ...
            console.log(di.data);
            return (
              <div className="col-md-4 col-xl-3" style={{ display: currentdate.getDate() == deviceValue[0].timestamp.substring(8, 10) && currentdate.getHours() == deviceValue[0].timestamp.substring(11, 13) ? deviceValue[0].timestamp.substring(14, 16) >= currentdate.getMinutes() - 5 ? '1' : 'none' : 'none' }}>
             ...
             <button type="button" className="read_more_bn"><Link style={{ textDecoration: 'none' }} className="link" to={`/dash_summary/showSensors/${di.data.id}`}>More Details</Link></button>
              </div>
              </div>
              </div>
            );
