...
class Inactivedevices extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
    ...
    try {
      this.setState({ loadingData: true }, async () => {
        setInterval(async () => {
          ...
          const filtered = await response.data.filter((dt) => dt.companyName === `${CompanyName}`);

          if (filtered.status === 500) {
            alert('Internal server error');
          }

          const ress = await axios.all(filtered.map((u) => axios.get(`http://192.168.0.194:5005/api/1.0/dashboard/devices/${u.id}/sensors/latest/calculated`)));
          if (ress.status === 500) {
            alert('Internal server error');
          }
          // console.log(ress);
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <div>
        <h3 style={{ marginLeft: '2%' }}>Inactive Devices:</h3>
        ...
        <div className="row">
          ...
          {this.state.sensorReadingTwo.map((di) => {
            ...
            console.log(di.data);
            return (
              <div className="col-md-4 col-xl-3" style={{ display: currentdate.getDate() == deviceValue[0].timestamp.substring(8, 10) && currentdate.getHours() == deviceValue[0].timestamp.substring(11, 13) ? deviceValue[0].timestamp.substring(14, 16) >= currentdate.getMinutes() - 5 ? 'none' : '1' : '1' }}>
              ...
              {this.state.displayindicator.map((readings) => (
                ...
                <button type="button" className="read_more_bn"><Link style={{ textDecoration: 'none' }} className="link" to={`/dash_summary/InactiveDevices/sensors/${di.data.id}`}>More Details</Link></button>
              </div>
              </div>
              </div>
            );
